class Solution:
    def max_product(self, nums: list) -> int:
        ...
        for i in nums[1:]:
            # use previous max/min*current i or restart from i. The absolute value of
            # the min could be larger so we store it.
            max_to_n = max(prev_max * i, prev_min * i, i)
            min_to_n = min(prev_max * i, prev_min * i, i)
            prev_max = max_to_n
            prev_min = min_to_n
